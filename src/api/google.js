...
const AUTO_COMPLETE_ENDPOINT =
    GOOGLE_MAP_API + "/place/autocomplete/json?key=" + API_KEY;
...
const autoComplete = location =>
    fetch(
        AUTO_COMPLETE_ENDPOINT +
            "&types=establishment&input=" +
            encodeURIComponent(location)
    )
        .then(res => {
            if (res.status >= 400) return Promise.reject(res.status);
            return res.json();
        })
        .then(response =>
            Promise.resolve(
                response.predictions.map(place => ({
                    id: place.id,
...
const findPlaceNearby = query =>
    fetch(
        PLACE_NEARBY +
            "&type=restaurant&location=-33.92399,151.22749&radius=500&query=" +
            query
    )
        .then(res => {
            if (res.status >= 400) return Promise.reject(res.status);
